...
public class SubMenuAdapter extends BaseAdapter {
	...
	public View getView(final int position, View view, final ViewGroup parent) {
		...
		try
    	{
    		...
    		if(c == null)
    		{
    			Metodos_auxiliares.mensagem("Erro ao iniciar os elementos", context);
    			return null;	
    		}
		...
		{
			Metodos_auxiliares.mensagem("Erro ao iniciar os elementos" + e.getMessage() + e.toString(), context);
		}
	...
	private Cursor carrega_sub_categorias(int cate_id)
	{
		...
		try		{    	
			...
			Db_mvc database = new Db_mvc(context);
			c = database.select("select suca_descricao, suca_ordem, suca_id as _id from subcategorias where suca_cate_id = " + String.valueOf(cate_id + 1) + " order by suca_ordem");
			if(c.moveToFirst())
			{
	    		do
	    		{
		...
		{
			Metodos_auxiliares.mensagem("Erro 01 ao executar a consulta " + e.getMessage(), context);
			c.close();
